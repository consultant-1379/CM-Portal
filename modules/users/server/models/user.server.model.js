...
UserSchema.pre('save', async function (next) {
  ...
  if (this.role_id === undefined) {
    try {
      ...
      if (!userRole) { // db empty, create roles
        userRole = new Role();
        userRole.name = 'user';
        userRole.save();
        var superAdminRole = new Role();
        superAdminRole.name = 'superAdmin';
        superAdminRole.pathsPermissions = [{ resources: '/*', methods: 'get put post delete' }];
        superAdminRole.save();
    ...
    } catch (userSchemaPresaveError) {
      return userSchemaPresaveError;
    }
  }
  next();
});
